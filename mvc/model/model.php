...
class model{
    ...
    public function AddBugItem($title, $descr, $file, $link) {
      mysql_set_charset('utf8', $link);
      $sql  = 'insert into bug_list (title, description, file, status, uid, date_create, allowed) ';
      $sql .='values("'.$title.'","'.$descr.'","'. $file.'","0","0", NOW(), "0")';
      $result = mysql_query($sql, $link);
      if (!$result)
      {
        print 'MySQL Error: ' . mysql_error();
    ...
    public function insertDemoLicense($link, $ip, $uid){
      mysql_set_charset('utf8', $link);
      $sql  = 'insert into license (ip, authcode, registercode, regdate, payment, date_pay, invoice_id, uid) ';
      $sql .='values("'.$ip.'", "DEMO", "DEMO", Now(), 0, Now(), 0, "'.$uid.'")';
      $result = mysql_query($sql, $link);
      if (!$result)
      {
        print 'MySQL Error: ' . mysql_error();
    ...
    public function getItemBug($link, $bid) {
      mysql_set_charset('utf8', $link);
      $sql  = 'select * from bug_list where id="'. $bid .'" order by id desc';
      $result = mysql_query($sql, $link);
      if (!$result){
        print 'MySQL Error: ' . mysql_error();
        exit;
    ...
    public function updateAuthCode($link, $ac, $lid) {
      mysql_set_charset('utf8', $link);
      $sql  = 'update license set authcode = "' .  $ac . '" where id = "' . $lid . '"';
      $result = mysql_query($sql, $link);
      if (!$result){
        print 'MySQL Error: ' . mysql_error();
        exit;
    ...
    public function getBugList($link) {
      mysql_set_charset('utf8', $link);
      $sql  = 'select * from bug_list order by id desc';
      $result = mysql_query($sql, $link);
      if (!$result){
        print 'MySQL Error: ' . mysql_error();
        exit;
    ...
    public function getLicensed($link, $uid){
      mysql_set_charset('utf8', $link);
      $sql  = 'select * from license where uid = "'.$uid.'"';
      $result = mysql_query($sql, $link);
      if (!$result){
        print 'MySQL Error: ' . mysql_error();
        exit;
    ...
    public function getDemoLicensed($link, $uid){
      mysql_set_charset('utf8', $link);
      $sql  = 'select * from license where uid = "'.$uid.'" and payment = 0';
      $result = mysql_query($sql, $link);
      if (!$result){
        print 'MySQL Error: ' . mysql_error();
        exit;
